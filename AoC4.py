# -*- coding: utf-8 -*-
"""
Created on Mon Dec  4 19:41:25 2017
...
def isValidA(passphrase):
    for i in range(0, len(passphrase)):
        for j in range(i, len(passphrase)):
            if passphrase[i] == passphrase[j] and i != j:
                return False
    
    return True    


def isValidB(passphrase):
    sorta = ' '
    sortb = ' '
    
    for i in range(0, len(passphrase)):
        for j in range(i + 1, len(passphrase)):
            sorta = ''.join(sorted(passphrase[i]))
            sortb = ''.join(sorted(passphrase[j]))
            if sorta == sortb:
                return False
...
def main():
    ...
    for line in inputa:
        ...
        line = list(placeholder)
        if isValidA(line):
            numberValidA += 1
        if isValidB(line):
            numberValidB += 1
    
    print("The Number of Valid Passphrases for Part One is: %d" %numberValidA)
    print("The Number of Valid Passphrases for Part Two is: %d" %numberValidB)
