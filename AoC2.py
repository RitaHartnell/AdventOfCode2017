# -*- coding: utf-8 -*-
"""
Created on Sat Dec  2 11:47:38 2017

@author: melso
"""
import numpy as np

multiline_input = """278	1689	250	1512	1792	1974	175	1639	235	1635	1690	1947	810	224	928	859
...
a1 = np.array(multiline_input.split("\n"))
a1item = a1.item(0)
ainfo = a1item.split("\t")
x = a1.size
y = len(ainfo)
...
checksum = 0
for i in range (0, x):
    currentbig = a.item(i,0)
    currentsmall = a.item(i,0)
    for j in range (0, y):
        if a.item(i,j) > currentbig:
            currentbig = a.item(i,j)
        if a.item(i,j) < currentsmall:
            currentsmall = a.item(i,j)
    diff = currentbig - currentsmall
